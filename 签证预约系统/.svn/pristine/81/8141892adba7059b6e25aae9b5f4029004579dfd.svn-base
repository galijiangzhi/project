...
class OrderWorkstationAdmin(object):
    def __init__(self):
        ...
        self.get_order()
    def get_order(self):
        rslt = db.select('select id from `order`')
        i=rslt[0]
        for i in rslt:
            t1 = Thread(target=self.open_orderworkstation, args=(i[0],))
            t1.start()
            print(i[0])

    def open_orderworkstation(self,order_id):
        orderworkstation=ApplOrder(order_id)
    #todo 开线程定期更新订单字典
#     def work(self):
