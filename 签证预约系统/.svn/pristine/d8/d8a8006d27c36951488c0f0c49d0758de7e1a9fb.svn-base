...
def get_dates(country_code, schedule_id, session, user_agent):
    ...
    r = req.get(req_url, headers=headers, proxies=g.PROXIES)
    if r.status_code != 200:
        if r.status_code == 429:
            raise WarningException("TOO-MANY", "取得日期过于频繁，只需要等几分钟就会自动恢复")
        else:
            msg = "取得日期时候出现网络操作错误。 返回status_code=%d, 返回信息%s" % (r.status_code, r.text[0:100])
            raise ErrorException("GET-DATES-ERR", msg)
    ...
    if time_table:
        for tr in time_table.find_all("tr"):
            ...
            if len(s) >= 3 and s[0] != "No":
                ...
                city_cd = db.city_tbl.get_city_code(place)
                formated_date = "%s-%s-%02d" % (year_str, g.MONTH[month_str], int(day_str))
                result.append([city_cd, formated_date])
...
def get_times(country_code, city_cd, schedule_id, appt_date, session, user_agent):
    ...
    if r.status_code != 200:
        if r.status_code == 429:
            raise WarningException("取得时分数据过于频繁，只需要等几分钟就会自动恢复", "TOO-MANY")
        else:
...
def schedule(country_code, city_cd, schedule_id, appt_date, appt_time, session, post_header, post_data):
    ...
    r = req.post(post_url, headers=post_header, data=post_data, proxies=g.PROXIES)
    if r.status_code != 200:
        if r.status_code == 429:
            raise WarningException("预约POST过于频繁，只需要等几分钟就会自动恢复", "TOO-MANY")
        else:
            raise ErrorException("预约POST时候出现网络操作错误。 返回status_code=%d, 返回信息%s" % (r.status_code, r.text[0:100]), "POST-ERR")
    ...
    if (r.text.find('successfully scheduled') != -1 or r.text.find('Successfully Scheduled') != -1):
        ...
        raise ErrorException(msg, "POST-NO-ERR-BUT-UNKNOWN")
...
def get_post_data(country_code, schedule_id, session, user_agent):
    ...
    for i in range(MAX_RETRY):
        ...
        # 连续多次失败的话，只能退出
        if i == MAX_RETRY - 1:
            raise ErrorException("尝试了多次，还是无法移动到预约页面获取Post基础数据。"
                                 "这是一个严重错误，该账户只能退出，需要人工接入检查原因。 "
                                 "返回status_code=%d, 返回信息%s" % (r.status_code, r.text[0:100]),
                                 "GET-POST-DATA-ERR")
        # 休息10秒再尝试
        time.sleep(10)
...
def logout(country_code, session, user_agent):

    req_url = "https://ais.usvisa-info.com/en-%s/niv/users/sign_out" % country_code
    headers = {
        "User-Agent": user_agent,
        "Referer": req_url,
        "Cookie": "_yatri_session=" + session
    ...
    if r.status_code != 200:
        raise WarningException(
                             "logout时候出错，不过可以忽视。 返回status_code=%d, 返回信息%s" % (r.status_code, r.text[0:100]), "LOGOUT-REQUEST-ERR")
