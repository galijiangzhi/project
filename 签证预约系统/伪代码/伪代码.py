...
'''
    工作： 
        yes 读取客户 创建列表 -- def 从数据库读取用户信息(self) 
        yes 登录所有客户 -- def login(self)
        yes 防止客户网页超时
        yes 刷新新客户（间隔五分钟）
        下单检测用户状态（是否可以下单）
                建议在数据库账号表中加一个账号状态 0未登录1已登录
                检测新用户就五分钟检索一次 为账号状态为0的账号开启线程
        yes 维护客户需求列表      
'''
    def __init__(self):
    def 启动(self):
    '''
        self.从数据库读取用户信息(self)
        self.all_login(self,self.user_list)
        while true:
            扫描数据库 查找账号状态为未登录的账号
            开启线程为未登录账号进行登录
    '''
    ...
    def 从数据库读取用户信息(self):
        self.user_list=数据库内用户信息(IVR,预约状态,预约日期,暂停预约,创建时间,更新时间)
        for i in user_list:
            查询i【ivr】的账号类型并记录 (IVR,预约状态,预约日期,暂停预约,创建时间,更新时间,账号类型)
    def all_login(self):
    '''
        while true：
        
            for user_list:
                开启线程登录 线程内容：self.登录和刷新（账号 密码 ivr）
                更改数据库中记录的账号状态为已登录
                将日期数据加入到日期字典
    '''
        数据库.查找所有待预约的订单
